...
static void test_slm_sched_model_bindings(void)
{
    ...
    const char *code =
        ...
        "local thresholds = slm.sched_model_status('thresholds')\n"
        "assert(thresholds == nil or type(thresholds) == 'table',\n"
        "       'sched_model_status(thresholds) returns nil or table')\n"
        "local rebalance = slm.sched_model_status('rebalance')\n"
        "assert(rebalance == nil or type(rebalance) == 'table',\n"
        "       'sched_model_status(rebalance) returns nil or table')\n"
        "local xgboost = slm.sched_model_status('xgboost')\n"
        "assert(xgboost == nil or type(xgboost) == 'table',\n"
        "       'sched_model_status(xgboost) returns nil or table')\n"
        "if blob ~= nil then\n"
        "  assert(blob.kind == 'mlp', 'status kind round-trips')\n"
        "  assert(type(blob.state) == 'string', 'status.state is string')\n"
        "  assert(type(blob.has_staged) == 'boolean', 'has_staged is bool')\n"
        ...
        "  assert(thresholds.kind == 'thresholds', 'thresholds status kind round-trips')\n"
        "  assert(type(thresholds.state) == 'string', 'thresholds status.state is string')\n"
        "  assert(slm.sched_model_activate('thresholds') == false,\n"
        "         'thresholds activate without staged blob should fail')\n"
        "  assert(slm.sched_model_rollback('thresholds') == false,\n"
        "         'thresholds rollback without prior blob should fail')\n"
        "  assert(slm.sched_model_clear('thresholds') == true,\n"
        "         'thresholds clear should succeed for known kind')\n"
        "  assert(rebalance.kind == 'rebalance', 'rebalance status kind round-trips')\n"
        "  assert(type(rebalance.state) == 'string', 'rebalance status.state is string')\n"
        "  assert(slm.sched_model_activate('rebalance') == false,\n"
        "         'rebalance activate without staged blob should fail')\n"
        "  assert(slm.sched_model_rollback('rebalance') == false,\n"
        "         'rebalance rollback without prior blob should fail')\n"
        "  assert(slm.sched_model_clear('rebalance') == true,\n"
        "         'rebalance clear should succeed for known kind')\n"
        "  assert(xgboost ~= nil, 'xgboost kind is registered')\n"
        "  assert(xgboost.kind == 'xgboost', 'xgboost status kind round-trips')\n"
        "  assert(type(xgboost.state) == 'string', 'xgboost status.state is string')\n"
        "  assert(slm.sched_model_activate('xgboost') == false,\n"
        "         'xgboost activate without staged blob should fail')\n"
        "  assert(slm.sched_model_rollback('xgboost') == false,\n"
        "         'xgboost rollback without prior blob should fail')\n"
        "  assert(slm.sched_model_clear('xgboost') == true,\n"
        "         'xgboost clear should succeed for known kind')\n"
        "end\n"
