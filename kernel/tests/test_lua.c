...
static void test_slm_sched_model_bindings(void)
{
    ...
    /*
     * Source split note: per-kind assertions are issued by a Lua
     * helper rather than open-coded 5×6 times. The earlier open-coded
     * form (PR #906) generated a single C string literal of ~4400
     * chars — over C99's 4095-char required minimum — which broke
     * every non-x86_64 build with `-Werror=overlength-strings`. The
     * helper preserves the same semantic coverage (status-shape +
     * activate/rollback/clear-without-staged for each kind) in well
     * under 4095 chars per literal.
     */
    const char *code =
        "local kinds = { 'mlp', 'ppo', 'config', 'thresholds',\n"
        "                'rebalance', 'xgboost' }\n"
        "local statuses = {}\n"
        "for _, k in ipairs(kinds) do\n"
        "  local s = slm.sched_model_status(k)\n"
        "  assert(s == nil or type(s) == 'table',\n"
        "         'sched_model_status('..k..') returns nil or table')\n"
        "  statuses[k] = s\n"
        "end\n"
        "local mlp_status = statuses.mlp\n"
        "if mlp_status ~= nil then\n"
        "  assert(type(mlp_status.has_staged) == 'boolean',\n"
        "         'has_staged is bool')\n"
        "  assert(type(mlp_status.has_active) == 'boolean',\n"
        "         'has_active is bool')\n"
        "  assert(type(mlp_status.has_rollback) == 'boolean',\n"
        "         'has_rollback is bool')\n"
        "  local function check_kind(k, s)\n"
        "    assert(s ~= nil, k..' kind is registered')\n"
        "    assert(s.kind == k, k..' status kind round-trips')\n"
        "    assert(type(s.state) == 'string',\n"
        "           k..' status.state is string')\n"
        "    assert(slm.sched_model_activate(k) == false,\n"
        "           k..' activate without staged blob should fail')\n"
        "    assert(slm.sched_model_rollback(k) == false,\n"
        "           k..' rollback without prior blob should fail')\n"
        "    assert(slm.sched_model_clear(k) == true,\n"
        "           k..' clear should succeed for known kind')\n"
        "  end\n"
        "  for _, k in ipairs(kinds) do check_kind(k, statuses[k]) end\n"
        "end\n"
        "assert(slm.sched_model_load('bogus_kind', '/mnt/files/nope')\n"
        "         == false, 'unknown kind should fail')\n"
        "slm.shell_exec('write /mnt/files/bad_sched.blob hello')\n"
        "assert(slm.sched_model_load('mlp', '/mnt/files/bad_sched.blob')\n"
        "         == false, 'invalid blob should fail to stage')\n"
