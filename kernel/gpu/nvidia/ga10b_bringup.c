...
{
    bar0_w32(0x00409840u, 0xFFFFFFFFu);  /* mailbox_clear_r(0) */
    ...
    /* Empirical timeout: ~1 ms FECS round-trip for STOP_CTXSW per
     * nvgpu source comments. 1M iterations is generous; tighten if
     * needed later. Each iteration is one MMIO read (~100 ns on
     * Tegra), so loop bound ≈ 100 ms wall time. */
    uint32_t mb0 = 0;
    int hit = 0;
