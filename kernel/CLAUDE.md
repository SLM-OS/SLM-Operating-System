...
**Cross-CPU dispatch status (April 16, 2026):** FULL. `bench smp`, `bench stealing`, and 14 of 15 integration tests pass reliably on Pi 5 hardware. The remaining flake — `test_work_stealing_distributes_load` — is documented as inherently timing-dependent (5 short tasks complete on CPU 1 before stealers on CPUs 2/3 can claim their share). Timer-based preemption on secondary CPUs still uses the cooperative path (CNTPCT-driven tick at yield points); hardware timer IRQ delivery remains a separate blocker tracked under #134. NC run queues, NC task table, NC current-task pointers. All tasks run with `DAIF.I=1` (no in-task timer preemption). See this file's "Pi 5 spinlock DRAM flush" section for what unblocked the last gap.
