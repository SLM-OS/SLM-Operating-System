...
static const struct help_entry help_entries[] = {
    ...
    HELP_TEXT("lua",
        ...
        "  slm.model_load(path[, n])   Load ONNX from VFS, returns index or -1\n"
        ...
        "  slm.ai_sched_decision(id)   {core, priority_adj, preempt, raw} or nil\n"
        "  slm.task_migrate(id, cpu)   Move a non-running task to CPU, returns bool\n"
        "  slm.task_create(name, fn)   Spawn Lua task from fn bytecode, returns id\n"
        "  slm.task_kill(id)           Terminate task, returns bool\n"
        "  slm.task_set_priority(id,p) Change priority 0..7, returns bool\n"
        "  slm.task_pin(id, cpu)       Pin task (cpu<0 clears), returns bool\n"
        "\n"
