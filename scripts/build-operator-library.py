...
def main() -> int:
    ...
    # Enforce two contracts the C parser doesn't (and shouldn't) re-
    # validate at runtime:
    #   1. No two entries share the same (op_kind, tier, dtype). The
    #      parser handles duplicates by "first match wins" but a real
    #      build with duplicates is almost certainly a manifest typo.
    #   2. SASS paths must resolve INSIDE the manifest's directory
    #      tree. Lifts the trust requirement from "the manifest
    #      author and every editor" to "just the manifest's
    #      directory" — `../../../etc/passwd` is rejected at build
    #      time even if the manifest is generated by another tool.
    ...
    for i, entry in enumerate(entries):
        ...
        triple = (op_kind, tier, dtype)
        if triple in seen_triples:
            raise SystemExit(
                f"manifest entry {i}: duplicate "
                f"(op_kind={entry['op_kind']}, tier={entry['tier']}, "
                f"dtype={entry['dtype']}). Each triple may appear "
                f"at most once."
            )
        seen_triples.add(triple)

        sass_path = (manifest_dir / entry["sass"]).resolve()
        try:
            sass_path.relative_to(manifest_dir)
        except ValueError:
            raise SystemExit(
                f"manifest entry {i}: SASS path {sass_path} resolves "
                f"outside the manifest directory {manifest_dir}. "
                f"Refusing to read out-of-tree files. Move the SASS "
                f"file into the manifest's directory tree or update "
                f"the manifest path."
            )
