...
| Networking | QEMU ARM64: virtio-mmio + lwIP + auto-DHCP. x86-64 QEMU: virtio-pci + lwIP + auto-DHCP (via `struct net_driver` abstraction). Jetson: none — #25 EQOS driver pending. Pi 5: none | None (Pi 5 still needs BCM GENET driver, #202) | #202 | `kernel/drivers/virtio_net.c`, `kernel/drivers/virtio_net_pci.c`, `kernel/include/net_driver.h`, `docs/networking-expansion-plan.md` |
