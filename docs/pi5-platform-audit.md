...
| Networking | QEMU ARM64: virtio-mmio + lwIP + auto-DHCP. x86-64 QEMU: virtio-pci + lwIP + auto-DHCP. Pi 5 hardware: Cadence MACB/GEM via RP1, DHCP + ping verified on pi-5-1 at 2-3 ms RTT. Jetson: none — #25 EQOS driver pending. | Cadence MACB driver is polling-only — IRQ path blocked by #134 (NS-EL1 delivery). TX completion via `USED`-bit poll with 100 ms budget; RX via `net_poll` + cache-invalidate. | #202 (closed), #25, #134 | `kernel/drivers/macb.c`, `kernel/drivers/virtio_net.c`, `kernel/drivers/virtio_net_pci.c`, `docs/networking.md` §"Cadence MACB/GEM Driver (Pi 5)" |
