...
| PR-5 | **Eviction shaders + producer.** Tracked as [#964](https://github.com/SLM-OS/SLM-Operating-System/issues/964). Weights come from the in-tree fp32 source `runtime/src/mm/eviction/generated/mlp_policy_f32.rs` via a new `scripts/eviction-extract-weights.py` (mirror of `sched-extract-weights.py`). Sole new SASS kernel is `sigmoid_fp32`; everything else is satisfied by the existing `gemm_fp32` + `add_bias_relu_fp32` (with `apply_relu=0` for the output add). | Jetson |
| PR-6 | **Eviction dispatch.** Same shape as PR-3 for the eviction Q-net. Tracked as [#965](https://github.com/SLM-OS/SLM-Operating-System/issues/965). Blocked by PR-5. | Jetson |
...
*Last updated: 2026-05-17 — PR-1 (op DAG + shape tables), PR-2 (sched launcher), PR-3 (sched dispatch + pipeline_kind discriminator + dispatch-cliff fix #651/#653), and PR-4 (eviction-trait scaffold) landed. PR-5 (eviction shaders + producer) and PR-6 (eviction dispatch) are now tracked as [#964](https://github.com/SLM-OS/SLM-Operating-System/issues/964) and [#965](https://github.com/SLM-OS/SLM-Operating-System/issues/965). Earlier deferral note (2026-05-07) cited "no compiled-in source to extract from like ai_weights_mlp.c"; that turned out to be wrong — `runtime/src/mm/eviction/generated/mlp_policy_f32.rs` is the equivalent compiled-in fp32 source (gated on `feature = "ai_eviction_models"`, imported by `scripts/import_eviction_weights.sh` as an explicit OPTIONAL deliverable). PR-5's extractor parses that Rust file the same way `scripts/sched-extract-weights.py` parses `ai_weights_mlp.c`. No sibling-repo coordination required.*
