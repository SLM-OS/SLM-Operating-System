...
| `slm.sched_model_status(kind)` | Runtime blob status for one scheduler model kind. Valid kinds: `"mlp"`, `"ppo"`, `"xgboost"` (added by #855), `"config"`, `"thresholds"`, `"rebalance"`. Returns `{kind, state, has_staged, has_active, has_rollback, staged?, active?, rollback?}` or `nil`. |
| `slm.sched_model_stage(kind, path)` | Admin surface only. Stage a blob for `kind` from a VFS path. The XGBoost cascade (`xgboost`) is the only kind that exceeds the static dense pool — its bytes are copied into Rust-side heap. Returns `true` on success, `false` on parse / kind / path failure. |
| `slm.sched_model_activate(kind)` | Admin surface only. Promote staged → active for `kind`. Returns `true` on success, `false` when no staged blob exists. |
| `slm.sched_model_rollback(kind)` | Admin surface only. Restore prior active for `kind`. Returns `true` on success, `false` when no rollback slot is filled. |
| `slm.sched_model_clear(kind)` | Admin surface only. Drop staged + active + rollback for `kind`. Returns `true` on success. |
...
| `slm.model_swap(index, path[, name])` | Admin surface only. Atomically replace the weights and graph of an already-loaded model with a new ONNX file. In-flight inferences hold a refcount lease on the OLD weight block and continue to read it safely; subsequent inferences see the NEW weights. Slot identity (pinned flag, GPU-dispatch toggle) is preserved across the swap; `use_count` resets. Returns 0 on success, -1 on argument / read error, -2 if the slot is empty, -3 if the slot's backend does not support swap (Hailo CCW re-upload tracked in [#532](https://github.com/SLM-OS/SLM-Operating-System/issues/532)), -4 on parse / alloc failure. |
