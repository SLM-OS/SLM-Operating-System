...
5. **Direct UEFI boot** — WIP. Full write-up in [docs/archive/investigations/jetson-uefi-direct-result.md](jetson-uefi-direct-result.md). As of 2026-04-17 probe session: UEFI loads the PE and enters at EL2+VHE (same as kexec, contradicts earlier EL1 conclusion). Two confirmed blockers remain: (a) boot.S's unconditional `msr hcr_el2, (E2H|RW|TGE)` clobbers other HCR_EL2 bits UEFI had set and hangs silently — fix is read-modify-write to preserve UEFI's bits; (b) UARTC MMIO at `0x0C280000` faults from the EFI-application context even at EL2, blocking the post-EBS UART probe — UEFI's EFI-app CBB permission profile differs from kexec-from-Linux despite same EL. Remaining sequenced work: (1) RMW the HCR_EL2 write, (2) skip or guard the UARTC probe under UEFI-direct, (3) primary_cpu / kernel_main re-verification, (4) approach B (real `.reloc` / PIC) for absolute-address correctness inside `kernel_main`.
