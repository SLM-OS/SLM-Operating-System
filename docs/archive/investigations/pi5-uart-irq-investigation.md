...
1. ~~Write MSIX_CFG after IACK~~ — Full ENABLE+IACK_EN to direct register after IACK. Reads back 0x9 but no fire. irq_count=0.
2. ~~No IACK_EN + fast ICR clear~~ — Engine doesn't fire for NEW characters. Only fires for PERSISTENT RTIM at ENABLE time.
3. ~~Blocking uart_getc on ring buffer~~ — Shell hangs (IRQ never fires). Not a polling race.
4. ~~Separate ENABLE then IACK_EN writes~~ — Linux rp1_irqchip does these as separate SET writes. Tried: no change. irq_count=0.
5. ~~MSI-X disable/enable toggle~~ — Disabled MSI-X cap, programmed table, re-enabled. Config space writes work from EL1. irq_count=0.
6. ~~brcmstb MSI target address~~ — Programmed table with 0x0F_FFFFFFFC (Linux's brcmstb address). irq_count=0.

**Conclusion:** MSIX_CFG engine doesn't fire TLPs for new interrupt assertions regardless of table address or MSIX_CFG write sequence. Engine only responds at ENABLE time for already-active interrupts.

**Root cause hypothesis:** Linux doesn't use MSIX_CFG for MSI-X generation. It uses `pci_alloc_irq_vectors()` which goes through the brcmstb PCIe MSI controller. The MSI-X TLPs are generated by the PCIe endpoint's standard MSI-X hardware (controlled by config space Enable), NOT by the RP1 MSIX_CFG registers. MSIX_CFG may only control interrupt ROUTING within the RP1 (which vectors get forwarded to the MSI-X engine), not the actual TLP generation.

**Linux driver analysis (from `drivers/mfd/rp1.c`):**
- Linux allocates 61 MSI-X vectors via `pci_alloc_irq_vectors(pdev, 61, 61, PCI_IRQ_MSIX)`
- brcmstb programs MSI-X table with its own target addr (`0x0F_FFFFFFFC` → BAR2 → DRAM doorbell)
- Each vector gets its own chained GIC interrupt via `pci_irq_vector(pdev, i)`
- MSIX_CFG ENABLE + IACK_EN is written per-vector to control RP1-internal routing
- The MSI-X TLP generation is handled by the PCIe endpoint hardware, not MSIX_CFG

**Implication for SLM-OS:** To get interrupt-driven UART RX, SLM-OS needs to either:
1. Implement a minimal brcmstb MSI controller (program BAR2 as DRAM doorbell, handle MSI writes in a GIC handler)
2. Use PCIe INTA (legacy interrupt) instead of MSI-X — but RP1 may not support INTA
3. Accept polling (current working state) — no functional impact, just a nice-to-have optimization
