...
| Area | State | Notes |
|---|---|---|
| Eviction blob format + store | ✅ partial | Parser, staging, activate, rollback, and clear exist; first-cut formats are now documented, but still not generalized. Deferred follow-up: 🎫 `#448` |
...
| Scheduler runtime models | ✅ partial | `mlp`, `ppo`, `config`, `thresholds`, and `rebalance` exist; live behavior is validated, but the format family is still narrow. Deferred follow-up: 🎫 `#449` |
...
| Operator workflow wrapper | ✅ partial | `slm-modelctl.py` now supports subcommands, autoload management, legacy compatibility, scheduler probes, and HTTP fetch via `--http-url`. Deferred follow-up: 🎫 `#451` |
| Persistence / autoload | ✅ partial | Managed boot autoload exists for current eviction/scheduler blob kinds; authoritative managed copies and config now live persistently on boot FAT media under `0:/slmstore/`, and `/mnt/files` now prefers a persistent LittleFS image at `0:/slmstore/files.lfs` with RAM fallback when boot FAT is unavailable. Deferred follow-up: 🎫 `#452`, 🎫 `#453`, 🎫 `#454` |
| HTTP / authenticated transport | ✅ partial | Plain-HTTP download path exists in-kernel with shell, Lua/admin, and `slm-modelctl.py --http-url`; SHA-256 checked fetch is supported, while HTTPS and signed-artifact hardening are deferred: 🎫 `#382`, 🎫 `#383` |
...
- generalize the runtime blob formats beyond the current first-cut
  shapes: 🎫 `#448`
...
- stronger corruption/recovery policy than “log and skip failed entry”
  during boot replay: 🎫 `#452`
- broader operational validation beyond the current boot-FAT-backed
  production path for `/mnt/files`: 🎫 `#453`
- boot policy beyond replaying the configured paths: 🎫 `#454`
...
| Phase | State | Notes |
|---|---|---|
| S1 — Scheduler blob format | ✅ partial | Dense models + `config` + `thresholds` + `rebalance` exist; the format family is not complete yet. Deferred follow-up: 🎫 `#449` |
...
| S4 — Scheduler control surface | ✅ partial | Shell + Lua exist; richer operator tooling is deferred: 🎫 `#451` |
...
- broader scheduler payload families beyond the current set are tracked
  separately: 🎫 `#449`
...
| F2 — Small native upload path | ✅ partial | `put`, `xput`, `slm-put.py`, and `slm-modelctl.py` are live; shell-line-aware chunking and resume are in place. Deferred follow-up: 🎫 `#450` |
...
| F5 — Stronger authenticated transport | ☐ pending | Deferred to broader security hardening: 🎫 `#382`, 🎫 `#383` |
...
- autoload-managed canonical copies now live under
  `0:/slmstore/autoload/`, while `/mnt/files/autoload/` remains the
  fallback system-managed area when boot FAT authority is unavailable
...
- a more purpose-built native ingress transport beyond the current
  shell-framed / hex-over-shell path: 🎫 `#450`
