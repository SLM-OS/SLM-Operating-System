...
**Status:** Milestones M1–M7 complete + every capstone-scope gap from `docs/x86-64-capstone-gap-closure-plan.md` is ✅ CLOSED including P1-2 hardware validation on test-pc (10/10 boot, 5/5 multi-task preemption, 7/7 SMP dispatch — see `docs/testing/x86-hw-validation-2026-04-15.md`). Full OS boots on real hardware with 8-CPU SMP, PCI enumeration, NVIDIA RTX 3050 GPU identification + VRAM access, and topic-based message routing (pub/sub IPC). Phase E (GPU compute via GSP-RM, post-capstone) is actively in progress: E1 (firmware embedding), E2 (shared VBIOS parser), E2.5 (FWSEC extraction), E3.1 (Falcon primitives), E3.2 (VFIO IOMMU DMA), E3.3 (nvfw container parser), E3.4 FWSEC bug audit, E3.4.d (Booter Load on SEC2), E3.4.e (GSP RISC-V startup) and E4 (RPC ring skeleton) all shipped. The audited FWSEC code paths still need a hardware re-test on test-pc to confirm WPR2 populates after the BOOTVEC=0 + CPUCTL_ALIAS fixes. See `docs/x86-64-capstone-gap-closure-plan.md` and #27.

> **Build note (issue #141 workaround):** `runtime/src/inference/mathf.rs` provides scalar `sqrtf` (Newton-Raphson) and `tanhf` (Padé(7,7)) replacing `libm::sqrtf`/`libm::tanhf` in the LayerNorm/RMSNorm/GELU kernels. Required because libm 0.2.13+'s f16 paths trip rustc-LLVM's soften-operand legalizer on `x86_64-unknown-none` and aborts codegen. See #141 for the root-cause fix path. The kernel-boot test suite (`rust_run_tests`) covers `mathf::sqrtf` and `mathf::tanhf` against known values on every platform.
