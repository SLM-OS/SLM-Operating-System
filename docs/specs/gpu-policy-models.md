...
*Last updated: 2026-04-27 — PR-1 (op DAG + shape tables), PR-2 (sched launcher), PR-3 (sched dispatch + pipeline_kind discriminator), and PR-4 (eviction-trait scaffold) landed. PR-5/PR-6 (eviction shaders + dispatch) deferred — the eviction MLP weights are runtime-loadable only (no compiled-in source to extract from like ai_weights_mlp.c), so PR-5 needs either a trained `.evi.bin` from the sibling slm-os-page-sim project or a synthetic-weight test path.*
