...
**Slow-client policy.** Per-client drop-oldest on the TX ring when full. Drops are line-aligned (the eviction walks forward to the next `\n`) so a consumer never sees a truncated record. Drop counts surface per-session via `tcp_telemetry_server_foreach` and as the global `samples_dropped` in `tcp_telemetry_server_get_stats`. **The server never blocks the publisher.** All telemetry publishers use `msg_router_publish_nowait` — fire-and-forget; a wedged client stalls only on its own TX ring, not on the publisher. Sample-coalescence in the single-slot wildcard mailbox is mitigated by the periodic pump explicitly draining `tcp_telemetry_server_poll()` between its three back-to-back publishes (see §14.11).

**Cumulative counter semantics.** The per-topic `published` counters surfaced via `slm.telemetry_stats()`, `telemetry stats`, and `admin_telemetry_get_feed_stats` count **publish attempts** — every call to the `record_*()` helper increments its counter exactly once, regardless of whether any subscriber matched the topic or successfully consumed the sample. This matches the §7.4 promise that the feed is fire-and-forget. Subscribers that need delivery counts read their own per-session `samples_sent` / `drops` from `tcp_telemetry_server_foreach`.
...
11. **Single-slot wildcard mailbox limits the periodic pump's drain coupling (PR #571 follow-up).** `msg_router`'s mailbox is one slot wide. The periodic pump fires three back-to-back nowait publishes (`tel.cpu` → `tel.stl` → `tel.mem`), and PR #571 mitigates the resulting overwrite-before-drain race by hardcoding a `tcp_telemetry_server_poll()` call between each publish. The mitigation is consumer-specific: a future second wildcard subscriber to `tel.*` (UDP emitter, Lua callback) would not get its mailbox drained between publishes and would see the same "only the last writer wins" behavior the original bug report described.
    * **Decided 2026-04-29 (PR #571):** ship the consumer-specific drain because today there is exactly one same-task wildcard subscriber. Two architecturally cleaner fixes are deferred until a second consumer actually shows up:
      - **Option A — `msg_router_dispatch_pending()`:** a generic primitive that the pump calls instead of `tcp_telemetry_server_poll`. The primitive walks every wildcard subscription and, for each one whose mailbox has `ready=1`, dispatches the sample through that subscription's registered consumer callback. Decouples the pump from any specific consumer at the cost of a callback-per-subscription registration.
      - **Option B — multi-slot ring-buffer mailboxes:** replaces the single-slot `Mailbox.data[60]` with an N-slot ring (e.g., N=8). Publisher writes head, consumer reads tail, full ring drops oldest. Architecturally right, but every existing msg_router consumer would need a re-test for the new sample-ordering / drop semantics. Heavier; right answer if dispatcher-rate `tel.aix` ever needs sub-tick ordering preservation.
