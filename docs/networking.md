...
| General USB host support beyond the current NIC path | ☐ Planned | Tracked in #384. The next step is to replace the current one-tier/root-hub-specific model with generic multi-device topology, hotplug, alternate-setting, and class-binding support. See `docs/archive/plans/usb-host-generalization-plan.md`. |
