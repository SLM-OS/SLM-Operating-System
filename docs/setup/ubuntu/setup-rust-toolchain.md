# Installing Rust with aarch64-unknown-none Target on Ubuntu
...
- `aarch64-none-elf-gcc` cross-compiler (see [ARM64 Toolchain Setup](setup-arm64-toolchain.md))
...
## Step 4: Add the Bare-Metal ARM64 Target
...
## Step 5: Install rust-src Component
...
Ensure `aarch64-none-elf-gcc` is installed and in PATH:

```bash
which aarch64-none-elf-gcc
```

If not found, see [ARM64 Toolchain Setup](setup-arm64-toolchain.md).
