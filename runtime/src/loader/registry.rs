...
fn prepare_onnx_payload(name: &[u8], data: &[u8]) -> Result<PreparedPayload, LoadError> {
    ...
    for i in 0..parsed.initializer_count {
        ...
        // Static buffer to avoid stack allocation (load is already heavy on stack).
        ...
        let src = if let Some(raw) = tensor.raw_data {
            ...
            // SAFETY: rust_model_load and rust_model_swap are invoked
            // serially from a single shell / Lua interpreter; the I64
            // buffer is not concurrently accessed in practice.
            unsafe {
