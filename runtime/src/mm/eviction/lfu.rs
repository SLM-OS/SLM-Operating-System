...
//!
//! Ported verbatim from `slm_os_integration/src/lfu.rs` in the sibling
//! `slm-os-page-sim` project. Decisions match the Python reference in
//! `src/policies/lfu.py` — pick the candidate with the smallest
//! `access_count`; ties are broken by older `last_access_time`
//! (LRU within the tied set).
