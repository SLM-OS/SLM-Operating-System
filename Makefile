...
# AI scheduler weight set (#884, sub-ticket of #61).
#   synthetic — Plan A synthetic-trained MLP/PPO weights (default).
#   real      — SLM-OS-fine-tuned weights from sibling repo's
#               --weights-suffix _real export. Requires
#               ai_weights_*_real.c to have been imported via
#               `scripts/import_ai_weights.sh --include-real`.
# Per #848, both sets are options; neither replaces the other.
AI_WEIGHTS ?= synthetic
...
# Boundary trace framework default mask values. Both default to 0
# (silent at boot). For capture of pre-shell phases (PCIe link train,
# fw boot, post-boot RPC handshake) set non-zero values here, OR
# pass `hailo_trace.phase=...` / `hailo_trace.mech=...` on the
# kernel command line via cmdline.txt → /chosen/bootargs. See
# kernel/ai_accel/hailo/hailo_trace.h for the enum→bit mapping.
# Example: HAILO_TRACE_PHASE_DEFAULT=0x07 HAILO_TRACE_MECH_DEFAULT=0x03
# arms phases LINKUP|FW_BOOT|POSTBOOT for MMIO + PCI cfg.
HAILO_TRACE_PHASE_DEFAULT ?= 0
HAILO_TRACE_MECH_DEFAULT ?= 0
...
# AI_WEIGHTS only affects the build when AI_SCHED=ON, so omit it from the
# signature otherwise — keeps signature-driven rebuild detection from
# tripping on a no-op variable change in non-AI builds.
AI_WEIGHTS_SIG := $(if $(filter ON,$(AI_SCHED)),AI_WEIGHTS=$(AI_WEIGHTS);,)
KERNEL_BUILD_SIGNATURE := PLATFORM=$(PLATFORM);BUILD_TYPE=$(BUILD_TYPE);AI_SCHED=$(AI_SCHED);$(AI_WEIGHTS_SIG)HAILO_WIRE_DEBUG=$(HAILO_WIRE_DEBUG);HAILO_TRACE_PHASE_DEFAULT=$(HAILO_TRACE_PHASE_DEFAULT);HAILO_TRACE_MECH_DEFAULT=$(HAILO_TRACE_MECH_DEFAULT);WORK_STEALING=$(WORK_STEALING);SECONDARY_PREEMPT=$(SECONDARY_PREEMPT);JETSON_HW_TICK=$(JETSON_HW_TICK);DISABLE_EVICTION=$(DISABLE_EVICTION);EVICTION_MODELS=$(EVICTION_MODELS);EVICTION_DEFAULT_POLICY=$(EVICTION_DEFAULT_POLICY);EMBED_DEMO_SCRIPTS=$(EMBED_DEMO_SCRIPTS);JETSON_EL1_SMOKE=$(JETSON_EL1_SMOKE);HAILO_FW_BLOB=$(HAILO_FW_BLOB);SCHEDULER_HEF_BLOB=$(SCHEDULER_HEF_BLOB);USER_HEF_BLOB=$(USER_HEF_BLOB);OPLIB_BLOB=$(OPLIB_BLOB);MNIST_DIGITS_DIR=$(MNIST_DIGITS_DIR)
KERNEL_KEXEC_BUILD_SIGNATURE := PLATFORM=$(PLATFORM);BUILD_TYPE=$(BUILD_TYPE);AI_SCHED=$(AI_SCHED);$(AI_WEIGHTS_SIG)HAILO_WIRE_DEBUG=$(HAILO_WIRE_DEBUG);WORK_STEALING=$(WORK_STEALING);SECONDARY_PREEMPT=$(SECONDARY_PREEMPT);JETSON_HW_TICK=$(JETSON_HW_TICK);DISABLE_EVICTION=$(DISABLE_EVICTION);EVICTION_MODELS=$(EVICTION_MODELS);EVICTION_DEFAULT_POLICY=$(EVICTION_DEFAULT_POLICY);EMBED_DEMO_SCRIPTS=$(EMBED_DEMO_SCRIPTS)
KERNEL_TEST_BUILD_SIGNATURE := PLATFORM=$(PLATFORM);BUILD_TYPE=$(BUILD_TYPE);AI_SCHED=$(AI_SCHED);$(AI_WEIGHTS_SIG)HAILO_WIRE_DEBUG=$(HAILO_WIRE_DEBUG);HAILO_TRACE_PHASE_DEFAULT=$(HAILO_TRACE_PHASE_DEFAULT);HAILO_TRACE_MECH_DEFAULT=$(HAILO_TRACE_MECH_DEFAULT);WORK_STEALING=$(WORK_STEALING);SECONDARY_PREEMPT=$(SECONDARY_PREEMPT);JETSON_HW_TICK=$(JETSON_HW_TICK);DISABLE_EVICTION=$(DISABLE_EVICTION);EVICTION_MODELS=$(EVICTION_MODELS);EVICTION_DEFAULT_POLICY=$(EVICTION_DEFAULT_POLICY);EMBED_DEMO_SCRIPTS=$(EMBED_DEMO_SCRIPTS)
...
$(KERNEL_BUILD_DIR)/Makefile:
	...
	$(CMAKE) -G "Unix Makefiles" -B $(KERNEL_BUILD_DIR) \
		...
		-DPLATFORM=$(PLATFORM) \
		$(if $(filter ON,$(AI_SCHED)),-DENABLE_AI_SCHEDULER=ON -DAI_WEIGHTS=$(AI_WEIGHTS)) \
		$(if $(filter ON,$(HAILO_WIRE_DEBUG)),-DHAILO_WIRE_DEBUG=ON) \
		-DHAILO_TRACE_PHASE_DEFAULT=$(HAILO_TRACE_PHASE_DEFAULT) \
		-DHAILO_TRACE_MECH_DEFAULT=$(HAILO_TRACE_MECH_DEFAULT) \
